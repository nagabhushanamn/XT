...
class Product extends Component {
    ...
    changeTab(tabIndex) {
        this.setState({ currentTab: tabIndex }, () => {
            if (tabIndex === 3) {
                ...
                actions.loadReviews(item._id)
            }
        })
    }
    handleBuy() {
        let { item, actions } = this.props;
        actions.buy(item, Number.parseInt(1, 10))
    }
    componentDidUpdate() {
        ...
        if (product)
            return (
                <div>
                    ...
                    &nbsp;
                        <input readOnly style={{ width: '50px' }} type="text" ref="qty" />
                </div>
            )
        else return null;
    }
    ...
    addNewReview(review) {
        let { item, actions } = this.props;
        actions.addNewReview(item._id, review)
    }
    renderTabPanel(item) {
